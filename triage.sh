...
encode_nvenc()
{
	...
	ffmpeg -y -i $input_file -b:v $bit_rate -c:v ${codec}_nvenc \
		-preset p1 -tune ull -g 1 -rc cbr -bufsize $buf_size $encoded_name >/dev/null
	ffmpeg -y -i $encoded_name $encoded_name.y4m >/dev/null
...
encode_vaapi()
{
	...
	ffmpeg -y -vaapi_device /dev/dri/renderD128 -i $input_file -c:v ${codec}_vaapi \
		-idr_interval 1 -g 1 -rc_mode CBR -b:v $bit_rate -bufsize $buf_size -vf format=nv12,hwupload $encoded_name >/dev/null
	ffmpeg -y -i $encoded_name $encoded_name.y4m >/dev/null
	psy-ex-scores.py $input_file $encoded_name.y4m -t 16 | ansi2txt > $stats
	rm $encoded_name.y4m
	echo "Stats in ${stats}, reference output in ${encoded_name}."
