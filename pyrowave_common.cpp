...
void WaveletBuffers::allocate_images()
{
	auto info = ImageCreateInfo::immutable_2d_image(
			aligned_width / 2, aligned_height / 2, VK_FORMAT_R16_SNORM);
...
void WaveletBuffers::init_block_meta()
{
	for (int level = DecompositionLevels - 1; level >= 0; level--)
	{
		for (int component = 0; component < NumComponents; component++)
		{
			...
			{
				int blocks_x_16x16 = (wavelet_img->get_width(level) + 15) / 16;
				int blocks_y_16x16 = (wavelet_img->get_height(level) + 15) / 16;
				int blocks_x_64x64 = (wavelet_img->get_width(level) + 63) / 64;
...
bool WaveletBuffers::init(Device *device_, int width_, int height_)
{
	...
	if (!device->supports_subgroup_size_log2(true, 5, 5) && !device->supports_subgroup_size_log2(true, 6, 6))
		return false;
